function [Objects] = ImageAnalysis_subc_lamp_PerField_treatment(ch1,ch2,ch3,InfoTableThis, PreviewPath)
...
%vol(ch3, 0, 500)
    ...
    NucleiMask = NucleiBlurred > 300; % vol(NucleiMask)
        if sum(NucleiMask(:))== 0
            Objects = {}; 
            return
        end
...
%         end% vol(SUBCMask)  
    ...
    if sum(ColoMask(:))> 0
            ...
            ColoMaskPreview_perimeter = imoverlay(ColoMaskPreview, bwperim(max(ColoMask, [], 3)), [0 0 1]);
            ...
            SavePathColoperiPreview = [PreviewPath, filesep, Objects.Barcode{:}, '_', num2str(Objects.ROW), '_', num2str(Objects.COL), '_', num2str(Objects.Field), '_Colo_peri.png'];
            imwrite(ColoMaskPreview, SavePathColoPreview)
            imwrite(ColoMaskPreview_perimeter, SavePathColoperiPreview)
            else
            Objects = {}; 
            return
    end
